	...
	class WC_Wishlists_Plugin {
		...
		public function init() {
			...
			if ( WC_Wishlist_Compatibility::is_wc_version_gte_2_6() ) {
                ...
                add_action('woocommerce_account_account-wishlists_endpoint', array($this, 'add_lists_to_account_page'));
			} else {
		...
		public function add_session_message() {
			...
			if ( ! is_page( WC_Wishlists_Pages::get_page_id( 'create-a-list' ) ) ) {
				if ( $session_items && count( $session_items ) ) {
					$message = sprintf( __( 'You have %s items ready to move to a new list.  <a href="%s">Create a list</a>', 'wc_wishlist' ), count( $session_items ), WC_Wishlists_Pages::get_url_for( 'create-a-list' ) );
