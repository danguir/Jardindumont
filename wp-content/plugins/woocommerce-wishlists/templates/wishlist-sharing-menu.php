...
if ($wishlist_items && count($wishlist_items)) :
	...
	<?php if (strstr($wishlist->get_wishlist_sharing(), 'Private') === false && ($e_email | $e_facebook | $e_twitter | $e_pinterest)) : ?>
		...
		<?php if ($e_email) : ?>
				<li class="wl-email"><a rel="nofollow" class="wl-email-button" href="#share-via-email-<?php echo $wishlist->id; ?>"><?php _e('Email', 'wc_wishlist'); ?></a></li>
		<?php endif; ?>
			<?php if ($e_facebook) : ?>
				<li class="wl-facebook">
					<a rel="nofollow" onClick="window.open('http://www.facebook.com/sharer.php?s=100&amp;p[title]=<?php echo $title; ?>&amp;p[summary]=<?php echo $summary; ?>&amp;p[url]=<?php echo $url; ?>&amp;p[images][0]=<?php echo $image; ?>', 'sharer', 'toolbar=0,status=0,width=548,height=325');" href="javascript: void(0)"><?php _e('Facebook', 'wc_wishlist'); ?></a>
				</li>
			<?php endif; ?>
			<?php if ($e_twitter) : ?>
				<li class="wl-twitter"><a rel="nofollow" target="_blank" href="<?php echo $twitter_url; ?>"><?php _e('Twitter', 'wc_wishlist'); ?></a></li>
			<?php endif; ?>
