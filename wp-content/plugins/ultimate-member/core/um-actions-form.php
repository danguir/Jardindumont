	...
	***/
	...
	function um_submit_form_errors_hook_( $args ){
	    ...
	    if ( get_post_meta( $form_id, '_um_profile_photo_required', true ) && ( empty( $args['profile_photo'] ) && empty( $um_profile_photo ) ) ) {
	        $ultimatemember->form->add_error('profile_photo', sprintf(__('%s ce champ est obligatoire.','ultimate-member'), 'Profile Photo' ) );
	    }
	   ...
	   if( isset(  $fields ) && ! empty(  $fields ) ){
			foreach( $fields as $key => $array ) {
				...
				if ( isset( $array['type'] ) && $array['type'] == 'checkbox' && isset( $array['required'] ) && $array['required'] == 1 && !isset( $args[$key] ) ) {
					$ultimatemember->form->add_error($key, sprintf(__('%s ce champ est obligatoire.','ultimate-member'), $array['title'] ) );
				}
				...
				if ( isset( $args[$key] ) ) {
					...
					if ( isset( $array['required'] ) && $array['required'] == 1 ) {
						if ( !isset($args[$key]) || $args[$key] == '' ) {
							$ultimatemember->form->add_error($key, sprintf( __('%s ce champ est obligatoire','ultimate-member'), $array['label'] ) );
						}
					}
