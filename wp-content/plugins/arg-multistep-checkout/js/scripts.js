(function($){	
	...
	$(document).ready( function() {
		...
		$('.argmc-wrapper').on('submit', '.login', function(e) {
			...
			data.push({name: 'action', value: 'login'}, {name: 'rememberme', value: $('#rememberme').is(':checked')}, {name: 'security', value: argmcJsVars.loginNonce});
		...
		$('.argmc-wrapper').on('submit', '.register', function(e) {
			...
			var data = form.serializeArray();			
			data.push({name: 'action', value: 'register'}, {name: 'security', value: argmcJsVars.registerNonce});
		...
		if ($('.coupon-placeholder').length > 0) {
